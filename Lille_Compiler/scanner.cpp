...
string scanner::get_current_ident_name()
{
...
void scanner::scan_string()
{
	...
	// INSERT CODE HERE.
        // Keep scanning characters until reaching a single double quote.
        //int quote_count=1;
        bool scanning=true;
        char cur = next_char;
        current_string_value="";
        ...
        get_char();
        cur=next_char;
        //If the next char is a quote and the following is not then it is an empty string.
        // This would be equivalent to: ""<chars>
        if (cur == '"' && following_char() != '"')
        {
           cerr << "Empty string." << endl;
           scanning = false;
        }
        // If any reason scanning should be stopped
        if (scanning == false)
        {
           current_line_number = line_number;
           current_pos_on_line = pos_on_line;
        ...
        //SCAN
        while (eoln_flag == false && scanning == true)
        {
           //If come across embedded quote, copy one over and get an additional character
           if (cur == '"' && following_char() == '"')
           {
              current_string_value+='"';
              get_char();
           ...
           //If you come across a single quote, terminate
           else if (cur == '"' && following_char() != '"')
           {
              scanning = false;
              current_line_number = line_number;
              current_pos_on_line = pos_on_line;
           ...
           {
              current_string_value+=cur;
              current_pos_on_line = pos_on_line;
           }
           // One iteration, new character
           if (scanning==true)
           {
              get_char();
              cur=next_char;
        ...
        if (scanning == true && eoln_flag == true)
        {
           current_line_number = line_number;
           current_symbol = new symbol(symbol::strng);
           
           // Previous line had the error if reached end of the line
           current_token = new token(current_symbol, --current_line_number, current_pos_on_line);
           current_token->set_string_value(current_string_value);
           this_token();
...
void scanner::scan_digit()
{
	...
	// Note that a unary operator such as -1 would be detected as a minus sign followed by a digit.
        ...
        char cur;
        bool scanning = true;
        int sym_type = 0; //0 is integer, 1 is real, and 2 is range
        int zeros=0;
        string powr = "";
        string tmp;
        ...
        while (scanning == true && eoln_flag == false)
        {
           //If a random character that is not e/E, terminate
           if (!isdigit(cur) && cur != 'e' && cur != 'E' && cur != '.')
           {
              scanning = false;
              current_pos_on_line = pos_on_line;
           }
           else if (cur == '.')
           {
              ...
              if (following_char() == '.')
              {
                 //If found a range, terminate and return token
                 //Do not add to build
                 scanning = false;
                 current_pos_on_line = pos_on_line;
                 //A range must begin with an integer
                 if (sym_type != 0)
                    error->flag(current_line_number, current_pos_on_line, 38);
                 //At token return we will recognize that the previous token was a range symbol and ensure that current is an integer
              }
              //It was an int, but now a real
              else if (isdigit(following_char()))
              {
                 //add period to build
                 build+=cur;
                 get_char();
                 cur=next_char;//This is a digit
                 //Throw an error if there has already been a period which indicates it was a real
                 //This may only occur once per real
                 if (sym_type != 1)
                    sym_type=1;
                 else
                 {
                    //There was already a period: sym_type was already equal to 1
                    scanning = false;
                    current_pos_on_line = pos_on_line;
              ...
              else
              {
                 ...
                 current_pos_on_line = pos_on_line;
                 scanning = false;
                 error->flag(current_line_number, current_pos_on_line, error_message(symbol::real_num)); 
              }
           }
           else if (cur == 'E' || cur == 'e')
           {
              //Add E (for consistency) and first character which may be +/-/digit
              build+='E';
              get_char();
              cur=next_char;
              if (eoln_flag == false && (cur == '-' || cur == '+' || isdigit(cur)))
              {
                 //Keep adding numbers until non digit
                 //There must be at least one number
                 build+=cur;
                 //If entered as +/- and the following char is not a digit
                 //Then throw an error as it is not completed minimum scientific notation
                 if (cur == '+' || cur == '-')
                 {
                    if (!isdigit(following_char()))
                    {
                       current_pos_on_line = pos_on_line;
                       if (sym_type == 0)
                          error->flag(current_line_number, current_pos_on_line, error_message(symbol::integer));
                       else if (sym_type == 1)
                          error->flag(current_line_number, current_pos_on_line, error_message(symbol::real_num));
                    }
                 }
                 while (isdigit(following_char()))
                 {
                    get_char();
                    cur=next_char;
                    build+=cur;
              ...
              {
                 //throw error E is followed by a random character
                 current_pos_on_line = pos_on_line;
                 scanning = false;
           ...
           //Process the character in a state machine here
           else
           {
              build+=cur;
              if (!isdigit(following_char()) && following_char() != 'e' && following_char() != 'E' && following_char() != '.')
              {
                 current_pos_on_line = pos_on_line;
                 scanning = false;
              }
        ...
        current_pos_on_line++;
        // If done scanning and there is something to return
        // This includes errors. There must be a return.
        if (scanning == false && build.length() != 0)
        { 
           try
           {
              //Integer
              if (sym_type == 0)
              {
                 for (int x=0; x<build.length(); x++)
                 {
                    //After E, start copying the power
                    if (start_counting)
                       powr += build[x];
                    //Keep track of number for E, store in tmp
                    else if (build[x] != 'E' && start_counting == false)
                       tmp+=build[x];
                    if (build[x] == 'E')
                       start_counting = true;
                 }
                 //If it is scientific notation
                 if (start_counting)
                 {
                    try
                    {
                       zeros = stoi(powr);
                       //Power is negative, treat as a real
                       if (zeros<0)
                          sym_type=1;
                       //Integer with a positive exponent
                       //Add zeros on the end for number of the power
                       else
                       {
                          //Recall tmp is number up until E
                    ...
                    {
                       cerr << "Scientific Notation exponent error at " << e.what() << endl;
                    }
                 }
                 //Checking again just in case scientific changes sym_type
                 if (sym_type == 0)
                 {
                    current_integer_value = stoi(build);
                    if (debugger)
                       cout << "Final Value before return: " << current_integer_value << endl;
                 }
              }
              //Not an else just in case scientific changes sym_type
              if (sym_type == 1) 
              {
                 //If post scientific notation conversion yields no decimal values - KEEP as real
                 current_real_value = stof(build);
      
                 if (debugger)
                    cout << "Final Value before return: " << current_real_value << endl;
              }
           }
           catch(exception &e)
           {
              cerr << "Too large when converting with " << e.what() << endl;
              error->flag(current_line_number, current_pos_on_line, 62); 
           }   

           //Is it real, integer, or range
           //Range symbol value is 23.
           //If the previous symbol is not a range symbol, then return as is
           if (current_token->get_sym() != 23)
           {
              if (sym_type == 0)
           ...
           //If the previous IS a range symbol then it must be an integer or error
           else
           {
              //Current MUST be an integer, expected
              if(sym_type != 0)
              {
                 error->flag(current_line_number, current_pos_on_line, 38);
              }
        ...
        {
           cerr << "Nothing to return. Error." << endl;
           error->flag(current_line_number, current_pos_on_line, 51); 
        }
}
...
// the symbol s, then the scanner discards the token and advances to the next token in the source file.
{
   if (error->recovery())
   {
      ...
      {
         get_token();
         //If it sees a recognizeable symbol
         if (s == current_token->get_sym())
         {
            get_token();
            error->out_of_recovery();
         }
      }
   }
      ...
      error->syntax(s, current_token, error_message(s));
}
