...
//calls flag and enters into recovery mode
void error_handler::syntax(symbol::symbol_type s, token* t, int message)
{
   if (not recovering)
   {
      flag(t, message);
...
//Sets recovering to false
void error_handler::out_of_recovery()
{
   recovering = false;
}

//Gets recovering boolean
...
// Array of error messages is initialized. This allows the language of the error messages to be changed easily.
{
	...
	error_message[84] = "Integer, real, boolean, or string expression expected.";
	error_message[85] = "Identifier is not assignable. Must be a variable or reference parameter.";
	error_message[86] = "Integer, real, or boolean variable expected.";
