...
void token::set_prog_value(string s)
{
   if (sym->get_sym() == symbol::program_sym)
      ...
      throw lille_exception("Illegal attempt to set prog_value in token");
}
...
void token::set_proc_value(string s)
{
   if (sym->get_sym() == symbol::procedure_sym)
      ...
      throw lille_exception("Illegal attempt to set proc_value in token");
}

//Set function name
void token::set_func_value(string s)
{
   if (sym->get_sym() == symbol::function_sym)
      func_value = s;
   else
      throw lille_exception("Illegal attempt to set func_value in token");
}

void token::print_token()
{
	cout << "TOKEN: " << sym->symtostr() << endl;
	if (sym->get_sym() == symbol::real_num)
		cout << "  Real Value: " << real_value << endl;
	else if (sym->get_sym() == symbol::integer)
		cout << "  Int Value: " << integer_value << endl;
	else if (sym->get_sym() == symbol::strng)
		cout << "  String Value: " << string_value << endl;
	else if (sym->get_sym() == symbol::identifier)
		cout << "  ID Value: " << identifier_value << endl;
	cout << "  Line No: " << line_number << endl << "  Pos on line: " << pos_on_line << endl;
}
