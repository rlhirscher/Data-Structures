...
private:
	bool debugging {false};                 // Set debugging to true to execute statement to help debug the scanner, otherwise set to false.
        bool debugger {false}; 			// Simple debugger for strings and digits. (don't enable both debuggers at the same time)
...
public:
    ...
    // Returns Identifier name as a string
    //string get_current_identifier_name();
